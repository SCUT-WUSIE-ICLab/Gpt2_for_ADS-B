...
'''
这个类负责对数据帧进行规范化处理，有不同的规范化类型可供选择，包括标准化、最小-最大规范化等。
提供了自定义的规范化参数，如均值、标准差、最小值和最大值。
'''
...
# 用于处理时间序列回归档案数据。
# 加载和预处理时间序列数据，包括选择特定的列、处理不同长度的序列、插值处理等。 
...
class ADS_BData(BaseData):
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, config=None, limit_size=None):
        self.config = config
        self.all_df, self.labels_df = self.load_single(root_dir, file_list=file_list, pattern=pattern)
        # print(len(self.all_df[0][0]))
        # print(self.labels_df.shape)

        # 创建唯一样本ID列表
        self.all_IDs = pd.Series(range(len(self.all_df)))

        # 如果限制了数据大小，则调整self.all_IDs和self.all_df
        if limit_size is not None:
            if limit_size > 1:
                limit_size = int(limit_size)
            else:  # 如果limit_size在 (0, 1] 之间，则将其解释为比例
                limit_size = int(limit_size * len(self.all_IDs))
            self.all_IDs = self.all_IDs[:limit_size]
            self.all_df = self.all_df.iloc[self.all_IDs]
        
        # 创建 feature_df
        # 假设 all_df 中所有列都是特征
        self.feature_df = self.all_df.copy()

        # 或者，如果需要排除某些列，可以选择性地创建 feature_df
        # 例如，排除前两列：
        # self.feature_df = self.all_df.iloc[:, 2:]

        # 设置最大序列长度
        # 假设每个样本的时间序列长度为 4800
        self.max_seq_len = 2400

        # 如果这是一个分类任务，创建 class_names
        if self.config.get('task') == 'classification':
            unique_labels = np.unique(self.labels_df)
            self.class_names = {index: label for index, label in enumerate(unique_labels)}
        else:
            self.class_names = None

    def load_single(self, root_dir, file_list=None, pattern=None):
        # mat = scipy.io.loadmat(filepath)
        # # 这里需要根据你的 .mat 文件中的键来调整
        # data = mat['data_key']  # 替换 'data_key' 为实际的键
        # labels = mat['label_key']  # 替换 'label_key' 为实际的键
        # 打开MATLAB v7.3文件
        data_path = [os.path.join(root_dir, f) for f in os.listdir(root_dir) if f.endswith('.mat')]
        with h5py.File(data_path[0], 'r') as file:
            # 加载'data'数据集
            data = np.array(file['data'][:])
            # print(data.shape) #[2,4800,40000]
            # print(type(data))
            # 加载'label'数据集
            labels = np.array(file['label'][:])

        # 根据需要进行数据处理，如转换格式、插值等
        # ...

        # 首先重塑data为 (40000, 2*4800)
        reshaped_data = data.reshape(data.shape[2], -1)
        # reshaped_data = np.transpose(data, (2,1,0))
        # print(reshaped_data.shape)
        
        return pd.DataFrame(reshaped_data), pd.Series(labels.flatten())
        # return reshaped_data, pd.Series(labels.flatten())


# 提供了一个工厂字典，用于根据数据集类型动态创建数据集对象。
