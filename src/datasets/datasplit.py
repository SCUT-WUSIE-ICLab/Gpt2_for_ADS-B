...
'''
用于分割数据集。
根据指定的验证方法和分割比例将数据集分为训练集、验证集和测试集。
支持随机种子设置，以确保分割的可重复性。
允许对测试集进行自定义或按比例随机分配。
返回测试集、训练集和验证集的索引。
'''
...
'''
一个工厂类，用于创建特定类型的数据分割器，例如StratifiedShuffleSplit或ShuffleSplit。
提供通用的分割方法接口，但具体的实现由子类完成。
'''
...
用于创建分层随机洗牌的分割。
保留类别比例，适用于分类问题。
提供split_testset和split_validation方法，分别用于创建测试集和执行验证集的分割。
'''
...
创建随机洗牌分割，不考虑样本标签。
适用于不需要考虑类别比例的情况。
同样提供split_testset和split_validation方法。
'''
