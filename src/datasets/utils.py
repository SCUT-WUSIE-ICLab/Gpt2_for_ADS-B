...
'''
这些功能在处理时间序列回归任务时非常有用，尤其是在涉及多个维度和不同长度的序列时。
它提供了灵活的方法来处理、缩放和标准化数据，从而确保数据适合进行机器学习建模。
'''
...
'''
对时间序列数据进行均匀缩放，以确保所有序列具有相同的长度。
参数 data 是原始序列，max_len 是目标长度。
'''
...
'''
从 .ts 文件中加载数据到 Pandas DataFrame。
可以选择将 X 和 y 值分别作为独立的 DataFrame (X) 和 numpy 数组 (y) 返回。
处理丢失值，将其替换为指定的值（默认为 'NaN'）。
'''
...
'''
处理数据，即将 DataFrame 转换为 numpy 数组。
参数 X 是原始数据，min_len 是调整后的长度，normalise 指定是否进行标准化处理。
使用均匀缩放来调整每个维度的长度，以及标准化或最小-最大缩放方法来规范化数据。
'''
