...
'''
这个类用于创建掩码（missingness mask），动态地为每个样本的特征添加噪声（掩码）。
这种方法通常用于数据增强和模型鲁棒性的提升。
'''
...
'''
这个类用于遮蔽（mask）特定的特征，通常用于时序数据的特征转换任务。
可以用于掩码预测等任务。
'''
...
'''
这两个函数用于处理批数据的加载，包括对数据进行填充、掩码处理等，以适应模型的输入要求。
'''
def collate_superv(data, max_len=None):
    ...
    # print("features[0].shape[-1]:",features[0].shape[-1])
    for i in range(batch_size): # 9600 4800
        end = min(lengths[i], max_len)
        # print("lengths[i], max_len", lengths[i], max_len)
        # print("features:", features[i])
        # print(len(features[i]))
        X[i, :end, 0] = features[i][:end]
...
class ClassiregressionDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        # print(X)
        # print(X.shape)
        # print(type(self.labels_df.loc[self.IDs[ind]]))
        # y = self.labels_df.loc[self.IDs[ind]].values  # (num_labels,) array
        y = np.array([self.labels_df.loc[self.IDs[ind]]])
